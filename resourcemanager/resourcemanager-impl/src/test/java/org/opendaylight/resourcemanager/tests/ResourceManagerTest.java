...
public class ResourceManagerTest extends AbstractConcurrentDataBrokerTest {
    ...
    public final @Rule MethodRule guice = new GuiceRule(new ResourceManagerTestModule());
    ...
    private static final Map<String, Class<? extends ResourceTypeBase>> RESOURCE_TYPES
            = new HashMap<String, Class<? extends ResourceTypeBase>>() {
                    {
    ...
    public void testGetResourcePools() throws Exception {
        for (Class<? extends ResourceTypeBase> resourceType : RESOURCE_TYPES.values()) {
            ...
            assertSuccessfulFutureRpcResult(result);
            for (AvailableIds availableIds : result
                    .getResult().getAvailableIds()) {
                assertTrue(availableIds.getStart() < availableIds.getEnd().longValue());
            }
