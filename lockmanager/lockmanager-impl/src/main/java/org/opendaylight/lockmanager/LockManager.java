...
public class LockManager implements LockManagerService {
    ...
    private void getLock(final InstanceIdentifier<Lock> lockInstanceIdentifier, final Lock lockData)
        ...
        for (int retry = 1;; retry++) {
            try {
                LockManagerUtils.lockSynchronizerMap.putIfAbsent(lockName, new CompletableFuture<>());
            ...
            java.util.Optional.ofNullable(LockManagerUtils.lockSynchronizerMap.get(lockName)).ifPresent(future -> {
                try {
                    future.get();
                } catch (InterruptedException | ExecutionException e) {
            ...
            LockManagerUtils.lockSynchronizerMap.remove(lockName);
        }
    }
