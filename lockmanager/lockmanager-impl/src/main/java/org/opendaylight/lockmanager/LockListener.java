...
public class LockListener extends AsyncClusteredDataTreeChangeListenerBase<Lock, LockListener>
    ...
    public LockListener(DataBroker broker) {
        this.broker = broker;
    }
    ...
    protected void remove(InstanceIdentifier<Lock> key, Lock remove) {
        ...
        LOG.info("Received add for lock {} : {}", lockName, remove);
        String lockNameSynchronizer = LockManagerUtils.getLockSynchronizerName(lockName);
        synchronized (lockNameSynchronizer) {
            lockNameSynchronizer.notifyAll();
        }
    }
