...
public class TerminationPointStateListener
        extends AsyncClusteredDataTreeChangeListenerBase<OvsdbTerminationPointAugmentation,
    ...
    @Inject
    public TerminationPointStateListener(final DataBroker dataBroker, final JobCoordinator coordinator, final
                                         OvsTunnelTopologyStateUpdateHelper ovsTunnelTopologyStateUpdateHelper) {
    ...
    protected InstanceIdentifier<OvsdbTerminationPointAugmentation> getWildCardPath() {
        return InstanceIdentifier.builder(NetworkTopology.class).child(Topology.class)
                .child(Node.class).child(TerminationPoint.class).augmentation(OvsdbTerminationPointAugmentation.class)
                .build();
    }
    ...
    protected void remove(InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
        ...
        if (TunnelUtils.TUNNEL_PORT_PREDICATE.test(tpOld.getName()) && ItmScaleUtils.isInternal(tpOld.getName())) {
    ...
    protected void update(InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
        ...
        if (TunnelUtils.TUNNEL_PORT_PREDICATE.test(tpNew.getName()) && ItmScaleUtils.isInternal(tpNew.getName())) {
            LOG.debug("Received Update DataChange Notification for ovsdb termination point {}", tpNew.getName());
            if ((SouthboundUtils.bfdMonitoringEnabled(tpNew.getInterfaceBfd())
                    != SouthboundUtils.bfdMonitoringEnabled(tpOld.getInterfaceBfd()))
    ...
    private class RendererStateUpdateWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        public List<ListenableFuture<Void>> call() throws Exception {
            ...
            return ovsTunnelTopologyStateUpdateHelper.updateTunnelState(terminationPointNew);
        }
    }
