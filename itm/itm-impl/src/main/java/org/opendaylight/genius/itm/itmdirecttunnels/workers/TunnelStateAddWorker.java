...
public final class TunnelStateAddWorker {
    ...
    public TunnelStateAddWorker(final DirectTunnelUtils directTunnelUtils) {
    ...
    public List<ListenableFuture<Void>> addState(TunnelStateInfo tunnelStateInfo)
        ...
        tunnelEndPointInfo = tunnelStateInfo.getTunnelEndPointInfo();

        if (tunnelEndPointInfo != null) {
            dpnTepConfigInfo = tunnelStateInfo.getDpnTepInterfaceInfo();
        }
        ...
        if (stateTnl != null) {
            handleTunnelMonitoringAddition(nodeConnectorId, stateTnl.getIfIndex(), interfaceName, portNo);
        }
    ...
    private void handleTunnelMonitoringAddition(NodeConnectorId nodeConnectorId,
                                                Integer ifindex, String interfaceName, long portNo) {
        BigInteger dpId = DirectTunnelUtils.getDpnFromNodeConnectorId(nodeConnectorId);
        directTunnelUtils.makeTunnelIngressFlow(dpId, portNo, interfaceName,
