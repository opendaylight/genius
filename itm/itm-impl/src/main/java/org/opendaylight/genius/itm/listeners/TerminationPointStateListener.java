...
import org.opendaylight.genius.itm.confighelpers.TepStateAddWorker;
...
public class TerminationPointStateListener implements DataTreeChangeListener<OvsdbTerminationPointAugmentation> {
    ...
    private void update(@Nonnull InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
        ...
        if (handleTpUpdate(tpOld, tpNew)) {
            LOG.debug("Creating TepState for ovsdb termination point {}", tpNew.getName());
            TepStateAddWorker tepStateAddWorker =
                new TepStateAddWorker(dataBroker, itmTepUtils, itmFlowUtils, identifier, tpNew);
            jobCoordinator.enqueueJob(ITMConstants.ITM_PREFIX + tpNew.getName(), tepStateAddWorker);
        }
    }
    ...
    private boolean handleTpUpdate(OvsdbTerminationPointAugmentation tpOld, OvsdbTerminationPointAugmentation tpNew) {
        ...
        if (!isTunnelZoneConfigured(tpNew)) {
            return false;
        }
