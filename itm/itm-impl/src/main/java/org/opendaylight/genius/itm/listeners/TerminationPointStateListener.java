...
public class TerminationPointStateListener implements DataTreeChangeListener<OvsdbTerminationPointAugmentation> {
    ...
    private void update(@Nonnull InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
        ...
        if (ItmUtils.TUNNEL_INTERFACE_TYPE_MAP.containsValue(tpNew.getInterfaceType())) {
            LOG.debug("Received Update/Add Notification for ovsdb termination point {}", tpNew.getName());
            TepStateUpdateWorker tepStateUpdateWorker = new TepStateUpdateWorker(dataBroker, identifier, tpNew);
            jobCoordinator.enqueueJob(ITMConstants.ITM_PREFIX + tpNew.getName(), tepStateUpdateWorker);
        }
    }

    private void remove(@Nonnull InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
                        @Nonnull OvsdbTerminationPointAugmentation tpOld) {
        if (ItmUtils.TUNNEL_INTERFACE_TYPE_MAP.containsValue(tpOld.getInterfaceType())) {
            LOG.debug("Received remove DataChange Notification for ovsdb termination point {}", tpOld.getName());
            TepStateRemoveWorker tepStateRemoveWorker = new TepStateRemoveWorker(dataBroker, identifier);
            jobCoordinator.enqueueJob(ITMConstants.ITM_PREFIX + tpOld.getName(), tepStateRemoveWorker);
        }
    }
    ...
    public void onDataTreeChanged(
        ...
        for (DataTreeModification<OvsdbTerminationPointAugmentation> change : changes) {
            ...
            switch (mod.getModificationType()) {
                case DELETE:
                    remove(identifier, mod.getDataBefore());
                    break;
                case SUBTREE_MODIFIED:
                case WRITE:
                    update(identifier, mod.getDataAfter());
                    break;
                default:
