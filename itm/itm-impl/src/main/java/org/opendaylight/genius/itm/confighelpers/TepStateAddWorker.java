...
public class TepStateAddWorker implements Callable<List<ListenableFuture<Void>>> {
    private static final Logger LOG = LoggerFactory.getLogger(TepStateAddWorker.class);
    private final DataBroker dataBroker;
    private final ItmFlowUtils itmFlowUtils;
    private final ItmTepUtils itmTepUtils;
    private final InstanceIdentifier<OvsdbTerminationPointAugmentation> tpIid;
    private final OvsdbTerminationPointAugmentation tp;

    public TepStateAddWorker(final DataBroker dataBroker,
        ...
        this.tpIid = identifier;
        this.tp = tp;
    }

    private CheckedFuture<Void, TransactionCommitFailedException> createTepStateOvs() {
        //TODO: Don't need Tepstate for nonOfTunnels, but use it in delete flows. revisit this.
        TepState tepState = itmTepUtils.createTepState(tpIid, tp);
        InstanceIdentifier<TepState> tsIid = itmTepUtils.createTepStateIdentifier(tepState.getKey().getTepIfName());
        LOG.debug("Creating TepState: {}", tepState);
        ITMBatchingUtils.write(tsIid, tepState, EntityType.DEFAULT_OPERATIONAL);
        if (itmTepUtils.isOfTunnel(tp)) {
            itmFlowUtils.addTunnelFlows(tepState);
        } else {
            itmFlowUtils.addTunnelFlows(tpIid, tp);
        }
