    ...
    protected Object doExecute() {
        if (interfaceManager.isItmDirectTunnelsEnabled()) {
            ...
            itmProvider.showBridges(ovsBridgeRefEntries.stream()
                    .filter(dpnId == null ? i -> true : i -> i.getDpid().equals(dpnId))
                    .collect(Collectors.groupingBy((x) -> x.getDpid(),
                            Collectors.mapping((x) -> x.getOvsBridgeReference(), Collectors.toSet()))));
        } else {
            ...
            itmProvider.showBridges(bridgeRefInfo.get().getBridgeRefEntry().stream()
                    .filter(dpnId == null ? i -> true : i -> i.getDpid().equals(dpnId))
                    .collect(Collectors.groupingBy((x) -> x.getDpid(),
                            Collectors.mapping((x) -> x.getBridgeReference(), Collectors.toSet()))));
        }
        return null;
    }
}
