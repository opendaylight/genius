...
public class SouthboundUtils {
    ...
    private static final Map<Class<? extends TunnelTypeBase>, Class<? extends InterfaceTypeBase>> TUNNEL_TYPE_MAP = new HashMap<Class<? extends TunnelTypeBase>, Class<? extends InterfaceTypeBase>>() {
        {
            ...
            put(TunnelTypeMplsOverGre.class, InterfaceTypeL3gre.class);
     ...
     * DS
     */
    public static void addAllPortsToBridge(OvsBridgeEntry bridgeEntry, DataBroker dataBroker,
        ...
        if (bridgeInterfaceEntries != null) {
            for (OvsBridgeTunnelEntry bridgeInterfaceEntry : bridgeInterfaceEntries) {
                ...
                Interface iface = TunnelUtils.getInterfaceFromConfigDS(portName, dataBroker);
                if (iface != null) {
                    IfTunnel ifTunnel = iface.getAugmentation(IfTunnel.class);
                    if (ifTunnel != null) {
                        addTunnelPortToBridge(ifTunnel, bridgeIid, iface, portName, dataBroker);
    ...
    public static void removeAllPortsFromBridge(BridgeEntry bridgeEntry, DataBroker dataBroker,
            InstanceIdentifier<OvsdbBridgeAugmentation> bridgeIid, OvsdbBridgeAugmentation bridgeNew, Interface iface,
            List<ListenableFuture<Void>> futures) {
