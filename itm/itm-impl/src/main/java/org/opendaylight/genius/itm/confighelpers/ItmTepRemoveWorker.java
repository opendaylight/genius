        ...
        if (dcGatewayIpList != null && !dcGatewayIpList.isEmpty()) {
            ...
            for (DPNTEPsInfo dpnTEPInfo : delDpnList) {
                ...
                if (tunnelEndPointsList.size() == 1) {
                    ...
                    logger.error("DPNTEPInfo not available in data store for dpnId" + dpnTEPInfo.getDPNID()
                            + ". Unable to delete external tunnel for dpn ");
                }
            }
            for (DcGatewayIp dcGatewayIp : dcGatewayIpList) {
                futures.addAll(ItmExternalTunnelDeleteWorker.deleteTunnels(dataBroker, idManagerService,  dpnDeleteList
                        , meshedDpnList, dcGatewayIp.getIpAddress(), dcGatewayIp.getTunnnelType()));
            }
        }
