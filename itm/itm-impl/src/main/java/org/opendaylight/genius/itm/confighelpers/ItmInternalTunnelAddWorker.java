...
public class ItmInternalTunnelAddWorker {
  ...
  private static final FutureCallback<Void> DEFAULT_CALLBACK =
    ...
    private static void build_tunnel_from( DPNTEPsInfo srcDpn,List<DPNTEPsInfo> meshedDpnList, DataBroker dataBroker,
            ...
            WriteTransaction t, List<ListenableFuture<Void>> futures, ItmConfig itmConfig) {
                ...
                if (!srcDpn.getDPNID().equals(dstDpn.getDPNID())) {
                    if (!ItmUtils.getIntersection(srcte.getTzMembership(), dstte.getTzMembership()).isEmpty()) {
                        ...
                        wireUpBidirectionalTunnel( srcte, dstte, srcDpn.getDPNID(), dstDpn.getDPNID(), dataBroker,
                                idManagerService,  mdsalManager, t, futures, itmConfig);
                    }
                }
            }
        }
    ...
    private static void wireUpBidirectionalTunnel( TunnelEndPoints srcte, TunnelEndPoints dstte,
        ...
        if (!wireUp( srcte, dstte, srcDpnId, dstDpnId, dataBroker, idManagerService, mdsalManager, tx, futures, itmConfig)) {
        ...
        if (!wireUp( dstte, srcte, dstDpnId, srcDpnId, dataBroker, idManagerService, mdsalManager, tx, futures, itmConfig)) {
    ...
    private static boolean wireUp(TunnelEndPoints srcte, TunnelEndPoints dstte,
                                  ...
                                  List<ListenableFuture<Void>> futures, ItmConfig itmConfig) {
        // Wire Up logic
        ...
        if (tunType.equals(TunnelTypeVxlan.class)) {
            wireUpLogicalGroupTunnel(srcte, dstte, srcDpnId, dstDpnId, dataBroker,
                    idManagerService, mdsalManager, t, itmConfig);
        }
        createTunnelInterace(srcte, dstte, srcDpnId, dstDpnId, tunType, trunkInterfaceName, t);
    ...
    private static void wireUpLogicalGroupTunnel(TunnelEndPoints srcte, TunnelEndPoints dstte,
                                                 BigInteger srcDpnId, BigInteger dstDpnId,
                                                 DataBroker dataBroker, IdManagerService idManagerService,
                                                 IMdsalApiManager mdsalManager, WriteTransaction tx,
                                                 ItmConfig itmConfig) {

        boolean tunnelAggregationEnabled = ItmTunnelAggregationConfigHelper.isTunnelAggregationEnabled(itmConfig);
        if (!tunnelAggregationEnabled) {
            logger.info("MULTIPLE_VxLAN_TUNNELS: wireUpLogicalGroupTunnel - not allowed configuration!");
            return;
        }
        //The logical tunnel interface be created only when the first tunnel interface on each OVS is created
        Collection<InternalTunnel> tunnelList = ItmUtils.itmCache.getAllInternalTunnel();
        for (InternalTunnel tnl : tunnelList) {
            if (tnl.getSourceDPN().equals(srcDpnId) && tnl.getDestinationDPN().equals(dstDpnId)) {
                logger.debug("MULTIPLE_VxLAN_TUNNELS: wireUpLogicalGroupTunnel - not first tunnel b/w srcDpnId {}"
                        + " dstDpnId {}", srcDpnId, dstDpnId);
                return;
            }
        }
        String tunTypeStr = ItmUtils.convertTunnelTypetoString(TunnelTypeLogicalGroup.class);
        String trunkInterfaceName = ItmUtils.getTrunkInterfaceName(idManagerService, srcte.getInterfaceName(),
                srcDpnId.toString(), dstDpnId.toString(), tunTypeStr);
        logger.debug("MULTIPLE_VxLAN_TUNNELS: add the logical tunnel group {} because a first tunnel"
                + " interface on srcDpnId {}:{} dstDpnId {}:{} is created", trunkInterfaceName, srcDpnId,
                srcte.getIpAddress(), dstDpnId, dstte.getIpAddress());

        createTunnelInterace(srcte, dstte, srcDpnId, dstDpnId, TunnelTypeLogicalGroup.class, trunkInterfaceName, tx);

        createInternalTunnel(srcDpnId, dstDpnId, TunnelTypeLogicalGroup.class, trunkInterfaceName, tx);

        ItmTunnelAggregationConfigHelper.createEgressSelectGroup(srcDpnId, trunkInterfaceName,
