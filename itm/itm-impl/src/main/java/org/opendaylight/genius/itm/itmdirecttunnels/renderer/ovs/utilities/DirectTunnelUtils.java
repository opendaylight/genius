...
public final class DirectTunnelUtils {
    ...
    @Inject
    public DirectTunnelUtils(DataBroker dataBroker, IdManagerService idManager, IMdsalApiManager mdsalApiManager,
    ...
    public Integer allocateId(String poolName, String idKey) throws InterruptedException, ExecutionException {
        AllocateIdInput getIdInput = new AllocateIdInputBuilder().setPoolName(poolName).setIdKey(idKey).build();
        RpcResult<AllocateIdOutput> rpcResult = idManager.allocateId(getIdInput).get();
        if (rpcResult.isSuccessful()) {
            return rpcResult.getResult().getIdValue().intValue();
        } else {
            LOG.error("RPC Call to Get Unique Id returned with Errors {}", rpcResult.getErrors());
            return INVALID_ID;
        }
    }
