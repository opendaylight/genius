...
public final class DirectTunnelUtils {
    ...
    public void makeTunnelEgressFlow(DpnTepInterfaceInfo dpnTepConfigInfo, BigInteger dpnId, String portNo,
                                      String interfaceName, int addOrRemoveFlow) {
        LOG.debug("make tunnel egress flow for {}", interfaceName);
        String flowRef =
        ...
        if (NwConstants.ADD_FLOW == addOrRemoveFlow) {
            matches.add(new NxMatchRegister(NxmNxReg6.class,
                    MetaDataUtil.getRemoteDpnMetadatForEgressTunnelTable(dpnTepConfigInfo.getGroupId())));
        ...
        Flow egressFlow = MDSALUtil.buildFlowNew(NwConstants.EGRESS_TUNNEL_TABLE, flowRef, 5, flowRef, 0, 0,
                NwConstants.COOKIE_ITM_EGRESS_TUNNEL_TABLE, matches, mkInstructions);

        if (NwConstants.ADD_FLOW == addOrRemoveFlow) {
            mdsalApiManager.batchedAddFlow(dpnId, egressFlow);
        } else {
            mdsalApiManager.batchedRemoveFlow(dpnId, egressFlow);
        }
    }
