...
public class VtepAddWorker implements Callable<List<ListenableFuture<Void>>> {
    ...
    public VtepAddWorker(DataBroker broker, final ItmTepUtils itmTepUtils, Vteps vtep, TunnelZone zone) {
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> createOvsdbTunnel() {
        ...
        TerminationPoint tp = itmTepUtils.createOvsdbTerminationPoint(vtep.getNodeId(),
            String.valueOf(vtep.getIpAddress().getValue()), zone);
        InstanceIdentifier<TerminationPoint> tpIid =
            itmTepUtils.createTerminationPointInstanceIdentifier(vtep.getNodeId(), tp.getTpId().getValue());
