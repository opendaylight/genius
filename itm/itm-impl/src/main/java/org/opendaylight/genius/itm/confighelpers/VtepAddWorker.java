...
public class VtepAddWorker implements Callable<List<ListenableFuture<Void>>> {
    ...
    public VtepAddWorker(DataBroker broker, Vteps vtep, TunnelZone zone) {
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> createHwvtepTunnel() {
        WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
        Class<? extends InterfaceTypeBase> ifType = ItmUtils.TUNNEL_INTERFACE_TYPE_MAP.get(zone.getTunnelType());
        return tx.submit();
    }
