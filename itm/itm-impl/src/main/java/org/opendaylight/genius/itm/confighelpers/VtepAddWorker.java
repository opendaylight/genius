...
public class VtepAddWorker implements Callable<List<ListenableFuture<Void>>> {
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> createVtep() {
        ...
        for (Vteps dstVtep : zone.getVteps()) {
            if (!dstVtep.getNodeId().equals(vtep.getNodeId())) {
                ...
                if (itmTepUtils.isVtepOvs(vtep)) {
                    // Currently we only create srcTeps for OVSDB, so ignore others
    ...
    private void createSrcTep(List<DstTep> dstTepList) {
        if (!itmTepUtils.isVtepOvs(vtep) || !zone.isOptionOfTunnel()) {
            // Currently we only create srcTeps for OVSDB and ofTunnels, so ignore others
            return;
        }
    ...
    private DstTep buildDstTep(Vteps srcVtep, Vteps dstVtep) {
        ...
        long ifIndex = intIfIndex != null ? intIfIndex.longValue() : 0;
        if (ifIndex == 0) {
        ...
        long groupId = itmTepUtils.allocateGroupId(dstVtep);
        if (groupId == itmTepUtils.INVALID_GROUP_ID) {
            LOG.error("FATAL_ERROR: Unable to get groupId for destination {}", dstVtep.getNodeId());
        }
