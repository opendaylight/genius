...
public class ItmTzInstanceRecoveryHandler extends
    ...
    private void recoverTransportZone(String entityId) throws InterruptedException {
        ...
        List<String> tunnelList = ItmUtils.getInternalTunnelsofTzone(entityId, dataBroker);
        ...
        if (tz != null) {
            ...
            AtomicInteger registeredEvents = new AtomicInteger(0);
            if (!tunnelList.isEmpty()) {
                for (String tunnelInterface : tunnelList) {
                    ...
                    eventCallbacks.onRemove(LogicalDatastoreType.OPERATIONAL, stListId, (unused) -> {
                        ...
                        recreateTZ(entityId, tz, tzII, tunnelList.size(), registeredEvents);
                        return DataTreeEventCallbackRegistrar.NextAction.UNREGISTER;
                    }, Duration.ofMillis(5000), (id) -> {
                            LOG.trace("recreate transportzone");
                            recreateTZ(entityId, tz, tzII, tunnelList.size(), registeredEvents); });
                }
            } else {
                recreateTZ(entityId, tz, tzII, tunnelList.size(), registeredEvents);
    ...
    private void recreateTZ(String entityId, TransportZone tz, InstanceIdentifier<TransportZone> tzII,
        ...
        if (registeredEvents.intValue() == sizeOfTunnelList) {
            ...
            LOG.error("Number of tunnel interface {} is not equal to number of callbacks {}",
                    sizeOfTunnelList, registeredEvents);
        }
    }
}