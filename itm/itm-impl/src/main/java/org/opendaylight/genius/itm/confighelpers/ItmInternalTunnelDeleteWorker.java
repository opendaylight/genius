...
public class ItmInternalTunnelDeleteWorker {
            ...
            for (DPNTEPsInfo srcDpn : dpnTepsList) {
                ...
                for (TunnelEndPoints srcTep : srcDpn.nonnullTunnelEndPoints()) {
                    ...
                    if (itmConfig.isUseOfTunnels()) {
                        deleteOFPort(tx, srcDpn.getDPNID()); // delete of port at its children
                    }
                                    ...
                                    if (ItmUtils.getIntersection(dstTep.getTzMembership(),
                                        ...
                                        if (interfaceManager.isItmDirectTunnelsEnabled()) {
                                            if (checkIfTepInterfaceExists(dstDpn.getDPNID(), srcDpn.getDPNID())) {
                                                ...
                                                if (itmConfig.isUseOfTunnels()) {
                                                    removeOFTunnelFromDst(tx, srcTep, dstTep, dstDpn.getDPNID());
                                                } else {
    ...
    private void removeOFTunnelFromDst(TypedReadWriteTransaction<Configuration> tx, TunnelEndPoints srcTep,
                                       TunnelEndPoints dstTep, BigInteger dstDpnId) {
        String revTunnelName = ItmUtils.getTrunkInterfaceName(dstTep.getInterfaceName(),
                dstTep.getIpAddress().getIpv4Address().getValue(),
                srcTep.getIpAddress().getIpv4Address().getValue(),
                srcTep.getTunnelType().getName());
        Interface iface = dpnTepStateCache.getInterfaceFromCache(revTunnelName);
        if (iface != null && iface.augmentation(IfTunnel.class) != null) {
            try {
                LOG.trace("Removing Reverse OF Tunnel {}", revTunnelName);
                removeOFTunnelConfiguration(tx, new ParentRefsBuilder().setDatapathNodeIdentifier(dstDpnId).build(),
                    iface.getName());
    ...
    private void removeOFTunnelConfiguration(TypedReadWriteTransaction<Configuration> tx, ParentRefs parentRefs,
        String interfaceName) throws ExecutionException, InterruptedException, OperationFailedException {
    ...
    private void deleteOFPort(TypedReadWriteTransaction<Configuration> tx, BigInteger dpId)
        throws ExecutionException, InterruptedException, OperationFailedException {
        if (dpId == null) {
            return;
        }
        LOG.info("removing OF PORT configuration for {}", dpId);
        ...
        String ofPortName = null;
        if (ovsBridgeEntryOptional.isPresent()) {
            OvsdbBridgeRef ovsdbBridgeRef = ovsBridgeEntryOptional.get().getOvsBridgeReference();
            if (ovsdbBridgeRef == null) {
                LOG.info("removing of OF PORT failed on {} due to missing bridge ref", dpId);
                return;
            }
        ...
        // delete all child tunnels that are bound to the OF Port
        final List<String> childTunnelList = dpnTepStateCache.getOfTunnelChildInfoFromCache(ofPortName);
        final ParentRefs parentRefs = new ParentRefsBuilder().setDatapathNodeIdentifier(dpId).build();
        for (String childTunnelName : childTunnelList) {
            removeOFTunnelConfiguration(tx, parentRefs, childTunnelName);
        }
    }
