        ...
        try {
            ...
            for (DPNTEPsInfo srcDpn : dpnTepsList) {
                ...
                for (TunnelEndPoints srcTep : srcDpn.getTunnelEndPoints()) {
                        ...
                        if (!srcDpn.getDPNID().equals(dstDpn.getDPNID())) {
                            for (TunnelEndPoints dstTep : dstDpn.getTunnelEndPoints()) {
                                ...
                                if (!ItmUtils.getIntersection(dstTep.getTzMembership(), srcTZones).isEmpty()) {
                                    if (checkIfTrunkExists(dstDpn.getDPNID(), srcDpn.getDPNID(), srcTep.getTunnelType(),
                                            dataBroker)) {
                                        // remove all trunk interfaces
                                        logger.trace(
                                                "Invoking removeTrunkInterface between source TEP {} , Destination TEP {} ",
                                                srcTep, dstTep);
                                        removeTrunkInterface(dataBroker, idManagerService, srcTep, dstTep,
                                                srcDpn.getDPNID(), dstDpn.getDPNID(), t, futures);
                                    }
                                }
                            }
                        }
                    ...
                    logger.trace("Tep Removal of TEP {} from DPNTEPSINFO CONFIG DS with Key {} " + srcTep,
                            srcTep.getKey());
                    ...
                    InstanceIdentifier<DPNTEPsInfo> dpnPath = InstanceIdentifier.builder(DpnEndpoints.class)
                            .child(DPNTEPsInfo.class, srcDpn.getKey()).build();
                    /*
                     * Optional<DPNTEPsInfo> dpnOptional =
                     * ItmUtils.read(LogicalDatastoreType.CONFIGURATION,
                     * dpnPath, dataBroker); if (dpnOptional.isPresent()) {
                     */
                    if (meshedEndPtCache.isEmpty()) {
                        // DPNTEPsInfo dpnRead = dpnOptional.get();
                        // remove dpn if no vteps exist on dpn
                        // if (dpnRead.getTunnelEndPoints() == null ||
                        // dpnRead.getTunnelEndPoints().size() == 0) {
