...
public class VtepRemoveWorker implements Callable<List<ListenableFuture<Void>>> {
    ...
    public VtepRemoveWorker(DataBroker broker, Vteps vtep, Class<? extends TunnelTypeBase> tunnelType) {
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> removeOvsdbTunnel() {
        ...
        String ifName = ItmUtils.getTrunkInterfaceName(null, vtep.getNodeId(), vtepIpStr, tunnelType.getName());
    ...
    private CheckedFuture<Void, TransactionCommitFailedException> removeHwvtepTunnel() {
        WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
        Class<? extends InterfaceTypeBase> ifType = TUNNEL_TYPE_MAP.get(tunnelType);
        return tx.submit();
    }
    ...
    public String toString() {
        return "VtepRemoveWorker  { " + "Vtep: " + vtep + "  TunnelType : " + tunnelType.getSimpleName() + " }" ;
    }
}
