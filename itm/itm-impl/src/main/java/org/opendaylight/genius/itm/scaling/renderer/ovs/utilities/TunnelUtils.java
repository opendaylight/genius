...
public class TunnelUtils {
    ...
    public static final Predicate<String> tunnelPortPredicate =
            portName -> tunnelPortPattern.matcher(portName).matches();
    ...
    public static StateTunnelList getTunnelFromOperDS (String tunnelName, DataBroker dataBroker) {
        StateTunnelList stateTnl = (StateTunnelList) DataStoreCache.get(ITMConstants.TUNNEL_STATE_CACHE_NAME, tunnelName);
        if (stateTnl != null) {
            return stateTnl;
        }
        InstanceIdentifier<StateTunnelList> stateTnlId = getStateTunnelListIdentifier(new StateTunnelListKey(tunnelName));
        Optional<StateTunnelList> tunnelOptional = ItmUtils.read(LogicalDatastoreType.CONFIGURATION, stateTnlId,
                dataBroker);
    ...
    public static Interface getInterfaceFromConfigDS (String tunnelName, DataBroker dataBroker) {
        ...
        if (endPointInfo != null) {
            ...
            if (dpnTepInfo != null){
                ...
                String description = String.format( "%s %s", ItmUtils.convertTunnelTypetoString(dpnTepInfo
                                .getTunnelType()), "Trunk Interface");
    ...
    public static StateTunnelList getTunnelFromOperationalDS (String tunnelName, DataBroker dataBroker) {
        ...
        if (stateTnl != null )
            ...
            InstanceIdentifier<StateTunnelList> stateTnlII  = ItmUtils
                    .buildStateTunnelListId(new StateTunnelListKey(tunnelName));
            Optional<StateTunnelList> tnlStateOptional = ItmUtils.read(LogicalDatastoreType.OPERATIONAL,stateTnlII , dataBroker);
    ...
    public static void makeTunnelIngressFlow(List<ListenableFuture<Void>> futures, IMdsalApiManager mdsalApiManager,
        ...
        String flowRef = TunnelUtils.getTunnelInterfaceFlowRef(dpnId,
        ...
        if (NwConstants.ADD_FLOW == addOrRemoveFlow) {
            ...
            short tableId = (dpnTepConfigInfo.getTunnelType().isAssignableFrom(TunnelTypeMplsOverGre.class))
                    ...
                    : dpnTepConfigInfo.isInternal() ? NwConstants.INTERNAL_TUNNEL_TABLE : NwConstants.DHCP_TABLE_EXTERNAL_TUNNEL;
    ...
    public static StateTunnelList addStateEntry( TunnelEndPointInfo tunnelEndPointInfo, String interfaceName,
        ...
        //if (tunnelEndPointInfo != null) {
            ...
            */
            ...
            DpnTepInterfaceInfo dpnTepInfo = ItmScaleUtils.getDpnTepInterfaceFromCache(new BigInteger(tunnelEndPointInfo.getSrcDpnId()),
            ...
            // As ITM Direct Tunnels deals with only Internal Tunnels..Relook at this when it deals with external as well
            ...
            boolean tunnelState = (operStatus.equals(org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.interfaces.rev140508.interfaces.state.Interface.OperStatus.Up)) ? (true):(false);

            StateTunnelListKey tlKey = new StateTunnelListKey(interfaceName);
            stlBuilder.setKey(tlKey).setTunnelInterfaceName(interfaceName).setOperState(tunnelOperStatus).setTunnelState(tunnelState)
