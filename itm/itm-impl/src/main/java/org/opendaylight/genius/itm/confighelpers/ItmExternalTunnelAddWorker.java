...
public class ItmExternalTunnelAddWorker {
        ...
        if (null != cfgDpnList) {
            for (DPNTEPsInfo teps : cfgDpnList) {
                ...
                String trunkInterfaceName = ItmUtils.getTrunkInterfaceName(idManagerService, interfaceName
                        , firstEndPt.getIpAddress().getIpv4Address().getValue(), extIp.getIpv4Address().getValue(), tunTypeStr);
                ...
                logger.debug(" Creating Trunk Interface with parameters trunk I/f Name - {}, parent I/f name - {},"
    ...
    public static List<ListenableFuture<Void>> buildTunnelsFromDpnToExternalEndPoint(DataBroker dataBroker,
        ...
        List<ListenableFuture<Void>> futures = new ArrayList<>();
        List<DPNTEPsInfo> cfgDpnList =
                dpnId == null ? ItmUtils.getTunnelMeshInfo(dataBroker) : ItmUtils.getDPNTEPListFromDPNId(dataBroker, dpnId);
    ...
    public static List<ListenableFuture<Void>> buildHwVtepsTunnels(DataBroker dataBroker,
                                                                   IdManagerService idManagerService,
                                                                   List<DPNTEPsInfo> cfgdDpnList, List<HwVtep> cfgdHwVteps) {
        List<ListenableFuture<Void>> futures = new ArrayList<>();
    ...
    private static void createTunnelsFromCSSinTransportZone(String zoneName, DPNTEPsInfo dpn, TunnelEndPoints tep,
                                                            IdManagerService idManagerService, List<ListenableFuture<Void>> futures,
                    ...
                    if (sub.getDeviceVteps() != null && !sub.getDeviceVteps().isEmpty()) {
                        for (DeviceVteps hwVtepDS : sub.getDeviceVteps()) {
                            ...
                            if (!wireUp(dpn.getDPNID(), tep.getPortname(), sub.getVlanId(),
                                ...
                                logger.error("Unable to build tunnel {} -- {}"
                                        , tep.getIpAddress(), hwVtepDS.getIpAddress());
                            }
                            //TOR-CSS
                            logger.trace("wire up {} and {}", hwVtepDS,tep);
                            if (!wireUp(hwVtepDS.getTopologyId(), hwVtepDS.getNodeId(), hwVtepDS.getIpAddress(), cssID
                                    , tep.getIpAddress(), sub.getPrefix(), sub.getGatewayIp(), tep.getSubnetMask()
                                    , transportZone.getTunnelType(), false, monitorProtocol, monitorInterval
                                    , idManagerService, dataBroker, futures, transaction)) {
                                logger.error("Unable to build tunnel {} -- {}"
                        ...
                        if (sub.getDeviceVteps() != null && !sub.getDeviceVteps().isEmpty()) {
                            for (DeviceVteps hwVtepDS : sub.getDeviceVteps()) {
                                ...
                                if (!wireUp(hwTep.getTopo_id(), hwTep.getNode_id(), hwTep.getHwIp()
                                        , hwVtepDS.getNodeId(), hwVtepDS.getIpAddress(), hwTep.getIpPrefix(), hwTep.getGatewayIP()
                                        , sub.getPrefix(), tunType,false,monitorProtocol, monitorInterval, idManagerService
                                        , dataBroker, futures, transaction)) {
                                    logger.error("Unable to build tunnel {} -- {}"
    ...
    private static boolean wireUp(String topoId, String srcNodeid, IpAddress srcIp, String dstNodeId, IpAddress dstIp,
        ...
        logger.debug(" Creating ExternalTrunk Interface with parameters Name - {}, parent I/f name - {}, "
                + "source IP - {}, destination IP - {} gateway IP - {}", tunnelIfName, parentIf, srcIp, dstIp, gwyIpAddress);
        Interface hwTunnelIf =
                ItmUtils.buildHwTunnelInterface(tunnelIfName, String.format("%s %s", tunType.getName(), "Trunk Interface")
                        , true, topoId, srcNodeid, tunType, srcIp, dstIp, gwyIpAddress, monitorEnabled, monitorProtocol,
                        monitorInterval);
