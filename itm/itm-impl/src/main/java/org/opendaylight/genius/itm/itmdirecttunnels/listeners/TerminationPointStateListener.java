...
public class TerminationPointStateListener
    ...
    public void update(InstanceIdentifier<OvsdbTerminationPointAugmentation> identifier,
        ...
        if (directTunnelUtils.TUNNEL_PORT_PREDICATE.test(tpNew.getName())
            ...
            if ((directTunnelUtils.bfdMonitoringEnabled(tpNew.getInterfaceBfd())
                    != directTunnelUtils.bfdMonitoringEnabled(tpOld.getInterfaceBfd()))
        ...
        if (tunnelBfdStatus != null && !tunnelBfdStatus.isEmpty()) {
            for (InterfaceBfdStatus bfdState : tunnelBfdStatus) {
                if (bfdState.getBfdStatusKey().equalsIgnoreCase(directTunnelUtils.BFD_OP_STATE)) {
                    ...
                    livenessState = directTunnelUtils.BFD_STATE_UP.equalsIgnoreCase(bfdOpState)
