...
public class ItmTepInstanceRecoveryHandler implements ServiceRecoveryInterface {
    ...
    private DPNTEPsInfo extractDPNTepsInfo(String entityId) {
        ...
        for (Subnets sub : transportZone.getSubnets()) {
            ...
            for (Vteps vtep : sub.getVteps()) {
                if (ipAddress.equals(vtep.getIpAddress().getIpv4Address().getValue())) {
                    ...
                    LOG.info(vtep.getDpnId() + "\t" + vtep.getPortname() + "\t" + sub.getVlanId() + "\t"
                            + sub.getPrefix() + "\t" + sub.getGatewayIp() + "\n");
                }
            }
        }
        ...
        TunnelEndPoints tunnelEndPoints = ItmUtils.createTunnelEndPoints(dpnId, new IpAddress(ipAddress.toCharArray()),
                portName, false , vlanId, new IpPrefix(prefix.toCharArray()), new IpAddress(gwAddress.toCharArray()) ,
                zones, transportZone.getTunnelType(), itmConfig.getDefaultTunnelTos());
