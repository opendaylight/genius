...
public class ItmTepInstanceRecoveryHandler implements ServiceRecoveryInterface {
    ...
    private DPNTEPsInfo extractDPNTepsInfo(String entityId) {
        ...
        for (Subnets sub : transportZone.getSubnets()) {
            ...
            for (Vteps vtep : sub.getVteps()) {
                if (ipAddress.equals(vtep.getIpAddress().getIpv4Address().getValue())) {
                    ...
                    prefix = sub.getPrefix().getIpv4Prefix().getValue();
                    gwAddress = sub.getGatewayIp().getIpv4Address().getValue();
                    flag = true;
                }
            }
        }
        ...
        if (!flag || dpnId == null) {
            LOG.error("IP address is not valid.");
            return null;
        }
        ...
        LOG.trace("Recovering TEP with parameters DPN ID: {}, Port Np: {}, vlanId: {}, ipaddress: {}, IpPrefix: {},"
                        + "Gateway IP: {}, TransportZone: {}, TunnelType: {} - ", dpnId, portName, vlanId,
                new IpAddress(ipAddress.toCharArray()), prefix, gwAddress, zones, transportZone.getTunnelType());

        //OfTunnels is false byDefault
        TunnelEndPoints tunnelEndPoints = ItmUtils.createTunnelEndPoints(dpnId,
                new IpAddress(ipAddress.toCharArray()), portName, false, vlanId, new IpPrefix(prefix.toCharArray()),
                new IpAddress(gwAddress.toCharArray()), zones, transportZone.getTunnelType(),
