...
public class OvsdbNodeListener extends AbstractSyncDataTreeChangeListener<Node> {
    ...
    public void update(@Nonnull InstanceIdentifier<Node> instanceIdentifier, @Nonnull Node originalOvsdbNode,
         ...
         * NOTE: Reistration is done before remove operation.
         */
        if (isTzChanged) {
            if (strNewDpnId == null || strNewDpnId.isEmpty()) {
                LOG.error("TEP {} cannot be added in tz movement case. DPID for bridge {} is NULL.",
                        newLocalIp, newDpnBridgeName);
                return;
            }
            ...
            eventCallbacks.onRemove(LogicalDatastoreType.CONFIGURATION, tunnelEndPointsIdentifier, (unused) -> {
                ...
                return DataTreeEventCallbackRegistrar.NextAction.UNREGISTER;
                    ...
                    LOG.info("TZ movement case: callback event timed-out for a deletion of {} from DpnTepsInfo.",dpnId);
                    addOrRemoveTep(finalTzName, strNewDpnId, finalJobKey, finalLocalIp,
                        finalDpnBridgeName, finalOfTunnel, true);
                });
        }
        // handle TEP-remove in remove case, tep-ip update case, tz update case, Bridge change case
        ...
        // handle TEP-add in add case, tep-ip update case, Bridge change case
        if (isLocalIpAdded || isDpnBrChanged || isLocalIpUpdated) {
            if (strNewDpnId == null || strNewDpnId.isEmpty()) {
