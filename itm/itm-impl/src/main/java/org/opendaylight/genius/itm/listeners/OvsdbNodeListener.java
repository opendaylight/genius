...
public class OvsdbNodeListener extends AsyncDataTreeChangeListenerBase<Node, OvsdbNodeListener> implements AutoCloseable{
    ...
    protected void add(InstanceIdentifier<Node> identifier, Node ovsdbNodeNew) {
        ...
        if (ovsdbNewNodeAugmentation != null) {
            ...
            OvsdbOtherConfigInfo newOtherConfigInfoObj = getOvsdbNodeOtherConfigs(ovsdbNodeNew);
            if (newOtherConfigInfoObj == null) {
                return;
            }
            // store other config required parameters
            tep_ip = newOtherConfigInfoObj.getTepIp();
            tzName = newOtherConfigInfoObj.getTzName();
            dpnBridgeName = newOtherConfigInfoObj.getDpnBrName();
            ...
            if (!tep_ip.isEmpty()) {
                ...
                LOG.trace("Ovs Node [{}] is not configured with TEP-IP.", ovsdbNodeNew.getNodeId().getValue());
            }

            // if br-name is not received, take br-int by default
            if (dpnBridgeName.isEmpty()) {
                dpnBridgeName = "br-int";
                LOG.trace("Bridge Name is not specified for Ovs Node [{}], setting 'br-int' as bridge.", ovsdbNodeNew.getNodeId().getValue());
            }
            newOtherConfigInfoObj.setDpnBrName(dpnBridgeName);

            LOG.trace("OpenvswitchOtherConfigs parameters in new Node: TEP-IP: {}, TZ name: {}, DPN Bridge Name: {}",
            ...
            if (ovsNodeToBridgesDpnIdMap.containsKey(newOvsNode)) {
                ...
                ovsNodeToBridgesDpnIdMap.put(newOvsNode, newOtherConfigInfoObj);
        ...
        if (ovsdbNewBridgeAugmentation != null) {
            ...
            LOG.trace("OvsdbNodeListener add() called for OvsdbBridgeAugmentation of bridge: {}.", dpnBridgeName);
            ...
            if (ovsNodeToBridgesDpnIdMap.containsKey(updatedOvsNode)) {
                ...
                if (ovsdbOtherConfigObj != null) {
                    ...
                    if (ovsdbOtherConfigObj.getDpnBrName().equals(dpnBridgeName)) {
                        LOG.trace("OvsdbBridgeAugmentation ADD: Received bridge (Bridge name: {}, datapath ID: {}) would be used for Tunneling.", dpnBridgeName, strDpnId);
                        ovsdbOtherConfigObj.setStrDpnId(strDpnId);
                    } else {
                        LOG.trace("OvsdbBridgeAugmentation ADD: Received bridge (Bridge name: {},  datapath ID: {}) is NOT configured for Tunneling.", dpnBridgeName, strDpnId);
                        return;
                    }
                } else {
    ...
    protected void update(InstanceIdentifier<Node> identifier, Node ovsdbNodeOld, Node ovsdbNodeNew) {
        ...
        LOG.trace("OvsdbNodeListener called for Ovsdb Node ({}) Update.", ovsdbNodeOld.getNodeId().getValue());
        ...
        if (oldOtherConfigInfoObj != null && newOtherConfigsInfoObj == null) {
            ...
            LOG.trace("OtherConfig is deleted from Ovsdb node: {}", ovsdbNodeOld.getNodeId().getValue());
        }
    ...
    public OvsdbOtherConfigInfo getOvsdbNodeOtherConfigs(Node ovsdbNode) {
        ...
        if (otherConfigInfoObj == null) {
            LOG.error("OvsdbOtherConfigInfo class object could not be created. System fatal error.");
            return null;
        }
