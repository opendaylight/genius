...
public class OvsdbNodeListener extends AsyncDataTreeChangeListenerBase<Node, OvsdbNodeListener>
    ...
    @Override protected void add(InstanceIdentifier<Node> identifier, Node ovsdbNodeNew) {
        String newTepIp = "", tzName = "", dpnBridgeName = "";
        String strDpnId = "";
        ...
        if (ovsdbNewNodeAugmentation != null) {
            ...
            strDpnId = ItmListenerUtils.getBridgeFromConfig(ovsdbNodeNew, dpnBridgeName, dataBroker);
            if (strDpnId == null) {
                LOG.error("TEP {} cannot be added. DPN-ID for bridge {} is NULL.", newTepIp,
                    dpnBridgeName);
                return;
            }
            ...
            OvsdbTepAddWorker
                addWorker = new OvsdbTepAddWorker(newTepIp, strDpnId, tzName, dataBroker);
            coordinator.enqueueJob(newTepIp, addWorker);
        } else {
            LOG.trace("Ovsdb node {} without OvsdbNodeAugmentation is received. Nothing to do.",
                ovsdbNodeNew.getNodeId().getValue());
        }
    }
    ...
    @Override protected void update(InstanceIdentifier<Node> identifier, Node ovsdbNodeOld,
        ...
        if (isTepIpAdded || isTzChanged || isDpnBrChanged) {
            // get Datapath ID for bridge
            strNewDpnId = ItmListenerUtils.getBridgeFromConfig(ovsdbNodeNew, newDpnBridgeName,
                dataBroker);
        ...
        if (isOtherConfigDeleted || isTepIpRemoved || isTzChanged || isDpnBrChanged) {
            strOldDpnId = ItmListenerUtils.getBridgeFromConfig(ovsdbNodeNew, oldDpnBridgeName,
                dataBroker);
