...
public class ItmListenerUtils {
    ...
    public static void removeUnknownTzTepIntoTepsNotHosted(String tzName, IpAddress tepIpAddress, BigInteger dpnId, DataBroker dataBroker) {
        ...
        if (unknownTz == null) {
            ...
            if (vtepList == null || vtepList.isEmpty()) {
                ...
                LOG.trace("Remove TEP in unknown TZ ({}) when no vtep-list in the TZ. Nothing to do.", tzName);
            } else {
                ...
                if (vtepFound) {
                    ...
                    LOG.trace("Remove TEP with IP ({}) in unknown TZ ({}) as new TEP into vtep-list in the TZ.", tepIpAddress, tzName);
                    //removeVtepIntoTepsNotHosted(tzName, dpnId, dataBroker);
                    if (vtepList.size() == 1) {
    ...
    public static void addVtepIntoTepsNotHosted(List<UnknownVteps> updatedVtepList, IpAddress tepIpAddress,
                                                      String tzName, BigInteger dpnId, DataBroker dataBroker) {
        //Create TZ node path
        ...
        LOG.trace("Adding TEP into unknown (TZ: {}, DPID: {}, TEP IP: {}) in ITM Config DS.", tzName, dpnId, tepIpAddress);
    ...
    public static void removeVtepIntoTepsNotHosted(String tzName, BigInteger dpnId, DataBroker dataBroker) {

        UnknownVtepsKey unknownVtepkey = new UnknownVtepsKey(dpnId);
        InstanceIdentifier<UnknownVteps> vTepPath =
                InstanceIdentifier.builder(TransportZones.class)
        ...
        LOG.trace("Removing TEP from unknown (TZ: {}, DPID: {}) from ITM Config DS.", tzName, dpnId);
        // remove vtep
    ...
    public static void removeTzFromTepsNotHosted(String tzName, DataBroker dataBroker) {
        ...
        ItmUtils.asyncDelete(LogicalDatastoreType.CONFIGURATION, tzTepsNotHostedTepPath, dataBroker, ItmUtils.DEFAULT_CALLBACK);
    }
