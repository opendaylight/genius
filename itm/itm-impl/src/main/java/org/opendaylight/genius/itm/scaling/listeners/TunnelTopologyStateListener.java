...
@Singleton
public class TunnelTopologyStateListener extends AsyncDataTreeChangeListenerBase<OvsdbBridgeAugmentation, TunnelTopologyStateListener> {
    ...
    @Inject
    public TunnelTopologyStateListener(final DataBroker dataBroker, final IInterfaceManager iInterfaceManager,
    ...
    protected void remove(InstanceIdentifier<OvsdbBridgeAugmentation> identifier, OvsdbBridgeAugmentation bridgeOld) {
        ...
        coordinator.enqueueJob(bridgeOld.getBridgeName().getValue(), rendererStateRemoveWorker, ITMConstants.JOB_MAX_RETRIES);
    }
    ...
    protected void update(InstanceIdentifier<OvsdbBridgeAugmentation> identifier, OvsdbBridgeAugmentation bridgeOld,
        ...
        if(oldDpid == null && newDpid != null){
            ...
            coordinator.enqueueJob(bridgeNew.getBridgeName().getValue(), rendererStateAddWorker, ITMConstants.JOB_MAX_RETRIES);
    ...
    private class RendererStateUpdateWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        public List<ListenableFuture<Void>> call() throws Exception {
            // If another renderer(for eg : CSS) needs to be supported, check can be performed here
            // to call the respective helpers.
            return OvsTunnelTopologyStateUpdateHelper.updateOvsBridgeRefEntry(instanceIdentifier,
