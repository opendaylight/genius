...
public class ItmManagerRpcService implements ItmRpcService {
    ...
    private ListenableFuture<GetEgressActionsForTunnelOutput> getEgressActionsForInterface(String interfaceName,
        Long tunnelKey, Integer actionKey) throws OperationFailedException, InterruptedException, ExecutionException {
...
/*            int remoteDpnId;
            ...
            //since dst-id of the remote dpn is required to program flow
            Optional<DpnsTeps> dpnTepsStateOptional =
                dpnTepStateCache.get(new BigInteger(tunnelEndPointInfo.getDstEndPointInfo()));
            if (dpnTepsStateOptional.isPresent()) {
                ...
                List<Action> actions = getEgressActionInfosForInterface(tunnelType,
                    dpnTepsStateOptional.get().getDstId(), tunnelKey, actionKeyStart).stream()
                    .map(ActionInfo::buildAction).collect(Collectors.toList());
