    ...
    public Future<RpcResult<java.lang.Void>> setBfdParamOnTunnel(SetBfdParamOnTunnelInput input) {
        ...
        if (dpnTepInterfaceInfo != null) {
            ...
            try {
                ...
                LOG.error("OVS Bridge for the DPN Id {} is not present", srcDpnId);
                String msg = String.format("OVS Bridge for the DPN Id %s is not present", srcDpnId);
                rpcResult.set(RpcResultBuilder.<Void>failed().withError(RpcError.ErrorType.APPLICATION, msg, error)
                    .build());
                return rpcResult;
            }
            futures = txRunner.callWithNewWriteOnlyTransactionAndSubmit(tx -> {
                directTunnelUtils.updateConfiguration(srcDpnId, remoteDpn, ovsBridgeRefEntry);
            ...
            Futures.addCallback(futures, new FutureCallback<Void>() {
                @Override
                ...
                public void onFailure(Throwable error) {
                    String msg = "Unable to configure BFD params for the srcDpnId " + srcDpnId
                        + "destinationDpnId " + destDpnId;
                    LOG.error("Unable to configure BFD params for the srcDpnId {} destinationDpnId {}",
                        srcDpnId, destDpnId);
                    rpcResult.set(RpcResultBuilder.<Void>failed().withError(RpcError.ErrorType.APPLICATION, msg, error)
                        .build());
                }
            ...
            LOG.error("DpnTepINfo is NULL for the srcDpnId {} destinationDpnId {}", srcDpnId, destDpnId);
            String msg = "Unable to configure BFD params as DpnTepINfo is NULL for the srcDpnId" + srcDpnId
                + " destinationDpnId " + destDpnId;
