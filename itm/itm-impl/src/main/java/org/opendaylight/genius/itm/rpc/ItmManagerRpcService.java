...
public class ItmManagerRpcService implements ItmRpcService {
    ...
    public Future<RpcResult<GetEgressActionsForTunnelOutput>>
        ...
        if (tunnelName == null) {
            return Futures.immediateFuture(RpcResultBuilder.<GetEgressActionsForTunnelOutput>failed().withError(RpcError.ErrorType.APPLICATION,
        ...
        if (!dpnTepStateCache.isInternal(tunnelName) && !interfaceManager.isItmDirectTunnelsEnabled()) {
            ...
            try {
                ...
                if (!result.get().isSuccessful()) {
                    ...
                    String errMsg = String.format("RPC Call to Get egress actions for interface {} returned with Errors {}",
                    ...
                    return Futures.immediateFuture(RpcResultBuilder.<GetEgressActionsForTunnelOutput>success().withResult(output.build()).build());
                }
            } catch (Exception e) {
        ...
        return Futures.immediateFuture(RpcResultBuilder.<GetEgressActionsForTunnelOutput>failed()
                .withError(RpcError.ErrorType.APPLICATION, "Error while performing getEgressActionForTunnel").build());
    }
    ...
    public Future<RpcResult<GetTunnelTypeOutput>> getTunnelType(GetTunnelTypeInput input) {
        String tunnelName = input.getIntfName();
        if (tunnelName == null) {
            return Futures.immediateFuture(RpcResultBuilder.<GetTunnelTypeOutput>failed().withError(RpcError.ErrorType.APPLICATION,
            ...
            try {
                ...
                if (!result.get().isSuccessful()) {
                    LOG.warn("RPC Call to Get tunnel type for interface {} returned with Errors {}", tunnelName,
                            result.get().getErrors());
                    String errMsg = String.format("RPC Call to Get tunnel type for interface {} returned with Errors {}",
                            tunnelName, result.get().getErrors());
                    ...
                    return Futures.immediateFuture(RpcResultBuilder.<GetTunnelTypeOutput>success().withResult(output.build()).build());
                }
            } catch (Exception e) {
        ...
        return Futures.immediateFuture(RpcResultBuilder.<GetTunnelTypeOutput>failed()
                .withError(RpcError.ErrorType.APPLICATION, "Error while fetching getTunnelType").build());
    }
