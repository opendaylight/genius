...
public class ItmManagerRpcService implements ItmRpcService {
    ...
    @Override
    public Future<RpcResult<Void>> setBfdParamOnTunnel(SetBfdParamOnTunnelInput input) {
        ...
        if (dpnTepInterfaceInfo != null) {
            ...
            } catch (ReadFailedException error) {
                String msg = String.format("Could not update BFD configuration due to failure to read OVS Bridge "
                    + "for the DPN Id: %d", srcDpnId);
                rpcResult.set(RpcResultBuilder.<Void>failed().withError(RpcError.ErrorType.APPLICATION, msg, error)
                    .build());
                return rpcResult;
            }
            return fromListenableFuture(LOG, input, () -> txRunner.callWithNewWriteOnlyTransactionAndSubmit(tx -> {
            ...
            })).withRpcErrorMessage(e -> setBfdParamOnTunnelErrorMessage(srcDpnId, e.getMessage()))
                .onFailureLogLevel(ERROR) // do not LOG error in this case!
                .onFailure(cause -> {
                    if (!(cause instanceof IllegalArgumentException)) {
                        LOG.error("RPC setBfdParamOnTunnelErrorMessage() failed; input = {}", input, cause);
                    }
                }).build();
        } else {
            String msg = String.format("Unable to configure BFD params as DpnTepINfo is NULL for the srcDpnId %d "
                + "and destination Dpn Id %d", srcDpnId, destDpnId);
            rpcResult.set(RpcResultBuilder.<Void>failed().withError(RpcError.ErrorType.APPLICATION, msg)
                .build());
            return rpcResult;
        }
    }

    private String setBfdParamOnTunnelErrorMessage(final BigInteger srcDpnId, final String dueTo) {
        return String.format("Failed to update BFD configuration for the DPN Id {%d} due to %s",
            srcDpnId, dueTo);
    }
