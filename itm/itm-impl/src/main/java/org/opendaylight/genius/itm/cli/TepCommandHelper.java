...
public class TepCommandHelper {
    ...
    public void showAllUnprocessedNCCaches() {
        Set<Entry<String, NodeConnectorInfo>> cacheContent = unprocessedNodeConnectorCache.getAllEntries();

        if (!cacheContent.isEmpty()) {
            System.out.println("Unprocessed Node Connector Cache");
            cacheContent.forEach(ncInfo -> {
                while (!ncInfo.getKey().isEmpty()) {
                    System.out.println(" KEY:  " + ncInfo.getKey() + " Value: " + ncInfo.getValue());
                }
                System.out.println("No data in cache for " + cacheContent);
            });
        }

        Set<Entry<String, Set<NodeConnectorInfo>>> ncEndPtCacheContent =
        ...
        if (!ncEndPtCacheContent.isEmpty()) {
            ...
            ncEndPtCacheContent.forEach(ncInfos -> {
