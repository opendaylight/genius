...
public class TepCommandHelper {
    ...
    public void createLocalCache(BigInteger dpnId, String portName, Integer vlanId, String ipAddress,
        ...
        try {
            ...
            if(session != null) {
                session.getConsole().println("Invalid IpAddress. Expected: 1.0.0.0 to 254.255.255.255");
                return;
            } else {
                    throw new TepException("Invalid IpAddress. Expected: 1.0.0.0 to 254.255.255.255");
            }
        }
        try {
            ...
            if(session != null) {
                session.getConsole().println("Invalid Subnet Mask. Expected: 0.0.0.0/0 to 255.255.255.255/32");
                return;
            } else {
                throw new TepException("Invalid Subnet Mask. Expected: 0.0.0.0/0 to 255.255.255.255/32");
            }
        }

        if (!validateIPs(ipAddress, subnetMask, gatewayIp)) {
            if(session != null) {
                session.getConsole().println("IpAddress and gateWayIp should belong to the subnet provided");
                return;
            } else {
                    throw new TepException("IpAddress and gateWayIp should belong to the subnet provided");
            }
        }
        ...
        if (tZonesOptional.isPresent()) {
            ...
            if(tZones.getTransportZone() == null || tZones.getTransportZone().isEmpty())
            {
                if(session != null) {
                    ...
                    throw new TepException("No teps configured");
                }
                return;
            }
    ...
    public void showState(TunnelList tunnels, boolean tunnelMonitorEnabled, CommandSession session) throws TepException {
        ...
        if (tunnelLists == null || tunnelLists.isEmpty()) {
            if(session != null) {
                ...
                throw new TepException("No Internal Tunnels Exist");
            }
            return;
        }
