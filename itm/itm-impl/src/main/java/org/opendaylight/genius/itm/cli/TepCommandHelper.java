...
public class TepCommandHelper {
    ...
    public void showAllUnprocessedNCCaches() {
        Set<Map.Entry<String, NodeConnectorInfo>> cacheContent = unprocessedNodeConnectorCache.getAllEntries();

        if (!cacheContent.isEmpty()) {
            System.out.println("Unprocessed Node Connector Cache");
            Iterator<Map.Entry<String, NodeConnectorInfo>> entries = cacheContent.iterator();
            while (entries.hasNext()) {
                Entry<String, NodeConnectorInfo> nc = entries.next();
                System.out.println(" KEY:  " + nc.getKey() + " Value: " + nc.getValue());
            }
            System.out.println("No data in cache for " + cacheContent);
        }

        Set<Map.Entry<String, Set<NodeConnectorInfo>>> ncEndPtCacheContent = unprocessedNodeConnectorEndPointCache.getAllEntries();

        if (!ncEndPtCacheContent.isEmpty()) {
            System.out.println("Unprocessed Node Connector End Point Cache");
            for (Entry<String, Set<NodeConnectorInfo>> ncInfos : ncEndPtCacheContent) {
                System.out.println(" KEY: " + ncInfos.getKey() + "  --  VALUE: " + ncInfos.getValue());
            }
