...
Proposed change
===============
ITM will create a default transport zone upon initialization and configured with:
...
1. ``genius-itm-config.xml`` configuation file is introduced newly into ITM
...
#. Verify mandatory parameters for TEP configuration on OpenDaylight as transport-zone
   name and tunnel-type
#. Verify ITM tunnel creation by configuring TEP parameters using REST
#. Verify default transport zone creation in OpenDaylight during bootup
...
#. Verify transport zone configured by OVS register with OpenDaylight but no tunnel formation
#. Verify tunnel formation initiates after ITM REST call on OpenDaylight with already registered
   transport zones by OVSs
#. Verify TEP transport zone change from OVS will move the TEP to corresponding
   transport-zone in OpenDaylight
#. Verify TEP delete from OVS will remove TEP from transport zone in OpenDaylight
#. Verify TEP configuration of dpn-br-name from OVS doesn't allow changes
   after connected to OpenDaylight
