...
Traffic between endpoints on the same DPN
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
No pipeline changes required

L3 traffic between endpoints in different DPNs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Endpoint originating the traffic (**Ingress DPN**):
"""""""""""""""""""""""""""""""""""""""""""""""""""
 ...
 * The default logical group service uses OF select group to load balance traffic between the tunnels.
  ...
  | FIB table (21) ``match: vpn-id=router-id,dst-ip=ep2-ip set dst-mac=ep2-mac tun-id=ep2-label reg6=logical-tun-lport-tag`` =>
...
Endpoint receiving the traffic (**Ingress DPN**):
"""""""""""""""""""""""""""""""""""""""""""""""""""
No pipeline changes required

  | Classifier table (0) =>
  | Internal tunnel Table (36) ``match:tun-id=ep2-label`` =>
  | Local Next-Hop group: ``set dst-mac=ep2-mac,reg6=ep2-lport-tag`` =>
  | Egress table (220) output to endpoint 2

L2 traffic between endpoints in different DPNs
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Endpoint originating the traffic (**Ingress DPN**):
"""""""""""""""""""""""""""""""""""""""""""""""""""
* Remote next hop group in the FIB table references a logical tunnel group.
  ...
  | ELAN base table (48) =>
  | ELAN SMAC table (50) ``match: elan-tag=vxlan-net-tag,src-mac=ep1-mac`` =>
  | ELAN DMAC table (51) ``match: elan-tag=vxlan-net-tag,dst-mac=ep2-mac set tun-id=ep2-lport-tag reg6=logical-tun-lport-tag`` =>
  | Egress table (220) ``match: reg6=logical-tun-lport-tag`` =>
...
No explicit pipeline changes required
  ...
  | Classifier table (0) =>
  | Internal tunnel Table (36) ``match:tun-id=ep2-lport-tag set reg6=ep2-lport-tag`` =>
  | Egress table (220) output to endpoint 2

SNAT traffic from non-NAPT switch
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Endpoint originating the traffic is non-NAPT switch:
""""""""""""""""""""""""""""""""""""""""""""""""""""
...
Traffic on NAPT switch punted to controller:
""""""""""""""""""""""""""""""""""""""""""""
No explicit pipeline changes required
