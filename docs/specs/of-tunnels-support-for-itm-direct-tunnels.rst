...
* Use case 8: On tep delete of a flow-based vtep, where BFD monitoring is enabled,
  ITM has to delete the p2p tunnel created for BFD monitoring.
* Use case 9: On tep add of an already deleted flow-based vtep where monitoring was
  previously enabled, ITM should re-create the p2p tunnel for monitoring.
* Use case 10: On tep delete, update all groups on all DPNs with drop action.
* Use case 11: The ITM group will get deleted only when a scale-in happens with
  an explicit trigger to remove the DPN.
* Use case 12: Applications will get the ITM group-id to program their respective remote
...
* Use case 11: Add of-tunnels through CLI
* Use case 12: Delete of-tunnels through CLI
* Use case 13: Display of-tunnels through CLI

Following use cases will not be supported:

* BFD based tunnel monitoring will not be supported on OF-tunnels. However, if BFD monitoring
  is required between two end-points, point to point tunnels can be still be created between them
...
* If a VM migrates while the controller cluster is down, from a BFD monitored source DPN to another one,
  the current solution will have a limitation that it will not delete the p2p tunnel created on the previous source.
