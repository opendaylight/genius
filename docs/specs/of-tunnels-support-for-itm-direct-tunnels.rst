...
#. ITM: When creating tunnel interfaces for this tep, if
   ...
   interface.
#. ITM: If ``option-of-tunnel:true`` and this is first tunnlel on this device,
...
#. If ``tunnel-remote-ip:true`` and this is *last* tunnel on this device,
   delete tunnel port in OVSDB. Else, delete relevant datastores which were populated in ITM.
...
Using point to point tnnel itself for Data Traffic whenever BFD monitoring gets enabled
