...
public class TransactionAdapterTest {
    ...
    public void testAdaptedWriteTransactionFailsOnInvalidDatastore() throws Exception {
        try {
            managedNewTransactionRunner.callWithNewWriteOnlyTransactionAndSubmit(Datastore.OPERATIONAL, writeTx -> {
                TransactionAdapter.toWriteTransaction(writeTx).put(CONFIGURATION, TEST_PATH, newTestDataObject());
            }).get();
    ...
    public void testAdaptedReadWriteTransactionFailsOnInvalidDatastore() throws Exception {
        try {
            managedNewTransactionRunner.callWithNewReadWriteTransactionAndSubmit(Datastore.OPERATIONAL, writeTx -> {
                TransactionAdapter.toReadWriteTransaction(writeTx).put(CONFIGURATION, TEST_PATH, newTestDataObject());
            }).get();
            fail("This should have led to an ExecutionException!");
        } catch (ExecutionException e) {
            assertThat(e.getCause() instanceof IllegalArgumentException).isTrue();
