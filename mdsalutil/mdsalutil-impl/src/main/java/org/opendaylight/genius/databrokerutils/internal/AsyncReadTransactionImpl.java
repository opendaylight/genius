...
public class AsyncReadTransactionImpl implements AsyncReadTransaction {
    ...
    protected final ReadTransaction tx;

    public AsyncReadTransactionImpl(ReadTransaction tx) {
        this.tx = tx;
    }
    ...
    public <T extends DataObject> Elser<Optional<T>> read(LogicalDatastoreType store,
        ...
        CompletableFuture<com.google.common.base.Optional<T>> java8guavaOptionalFuture
            = new ListenableToCompletableFutureWrapper<>(guavaFuture);
        ...
        return new ElserImpl<>(java8Future.whenComplete((optional, exception) -> {
            optional.ifPresent(dataObject -> reader.accept(dataObject));
            if (exception != null) {
                // TODO LOG with original caller call trace how-to?
                LOG.error("Failed reading data from {} store path {}", store, path, exception);
            }
        }));
    }
