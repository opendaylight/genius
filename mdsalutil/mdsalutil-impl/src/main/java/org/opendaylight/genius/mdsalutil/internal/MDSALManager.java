...
public class MDSALManager extends AbstractLifecycle implements IMdsalApiManager {
    ...
    private String getKey(long groupId, BigInteger dpId) {
        String synchronizingKey = "key-" + groupId + dpId;
        return synchronizingKey.intern();
    }

    private String getKey(BigInteger dpId, short tableId, FlowKey flowKey) {
        String synchronizingKey = "key-" + dpId + tableId + flowKey;
        return synchronizingKey.intern();
    }
    ...
    public void syncSetUpFlowInternal(FlowEntity flowEntity, long delay, boolean isRemove) {
        ...
        if (isRemove) {
            String synchronizingKey = getKey(dpId, tableId, flowKey);
            synchronized (synchronizingKey.intern()) {
                if (flowExists(dpId, tableId, flowKey)) {
    ...
    public void syncSetUpGroupInternal(GroupEntity groupEntity, long delayTime, boolean isRemove) {
        ...
        if (isRemove) {
            String synchronizingKey = getKey(groupId, dpId);
            synchronized (synchronizingKey.intern()) {
                if (groupExists(dpId, groupId)) {
