...
import static org.opendaylight.genius.datastoreutils.listeners.DataTreeEventCallbackRegistrar.UnregisterOrCallAgain.UNREGISTER;
...
public interface DataTreeEventCallbackRegistrar {
    ...
    enum UnregisterOrCallAgain {
        /**
         * When the callback function returns this, then it is unregistered and will never be invoked anymore.
         */
        UNREGISTER,

        /**
         * When the callback function returns this, then it will be called again for add, update OR remove
         * on the given store &amp; path.
         */
        CALL_AGAIN
    }
    ...
    default <T extends DataObject> void onAdd(LogicalDatastoreType store, InstanceIdentifier<T> path,
            Consumer<T> callback) {
        onAdd(store, path, (Function<T, UnregisterOrCallAgain>) t -> {
            callback.accept(t);
            return UNREGISTER;
        });
    }
    ...
    default <T extends DataObject> void onUpdate(LogicalDatastoreType store, InstanceIdentifier<T> path,
        ...
        onUpdate(store, path, (BiFunction<T, T, UnregisterOrCallAgain>) (t1, t2) -> {
    ...
    default <T extends DataObject> void onRemove(LogicalDatastoreType store, InstanceIdentifier<T> path,
            Consumer<T> callback) {
        onRemove(store, path, (Function<T, UnregisterOrCallAgain>) t -> {
            callback.accept(t);
            return UNREGISTER;
        });
    }
