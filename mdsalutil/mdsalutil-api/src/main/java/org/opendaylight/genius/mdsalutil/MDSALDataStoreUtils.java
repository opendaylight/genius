...
public class MDSALDataStoreUtils {
    ...
    public static <T extends DataObject> CheckedFuture<Void, TransactionCommitFailedException> asyncRemove(
            final DataBroker broker, final LogicalDatastoreType datastoreType, InstanceIdentifier<T> path,
            FutureCallback<Void> callback) {
        WriteTransaction tx = broker.newWriteOnlyTransaction();
