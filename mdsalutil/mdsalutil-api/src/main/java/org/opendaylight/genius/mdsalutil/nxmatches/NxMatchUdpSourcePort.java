...
public class NxMatchUdpSourcePort extends NxMatchInfoHelper<NxmOfUdpSrc, NxmOfUdpSrcBuilder> {
    ...
    public NxMatchUdpSourcePort(int port, int mask) {
        super(NxmOfUdpSrcKey.class, NxMatchFieldType.nx_udp_src_with_mask, new long[] {port, mask});
        this.port = port;
        this.mask = mask;
    }
    ...
    public int compareTo(MatchInfoBase other) {
        return compareTo(other, new Comparator<NxMatchUdpSourcePort>() {
            @Override
            public int compare(NxMatchUdpSourcePort left, NxMatchUdpSourcePort right) {
