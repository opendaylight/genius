...
public class NxMatchTcpDestinationPort extends NxMatchInfoHelper<NxmOfTcpDst, NxmOfTcpDstBuilder> {
    ...
    public NxMatchTcpDestinationPort(int port, int mask) {
        super(NxmOfTcpDstKey.class, NxMatchFieldType.nx_tcp_dst_with_mask, new long[] {port, mask});
        this.port = port;
        this.mask = mask;
    }
    ...
    public int compareTo(MatchInfoBase other) {
        return compareTo(other, new Comparator<NxMatchTcpDestinationPort>() {
            @Override
            public int compare(NxMatchTcpDestinationPort left, NxMatchTcpDestinationPort right) {
