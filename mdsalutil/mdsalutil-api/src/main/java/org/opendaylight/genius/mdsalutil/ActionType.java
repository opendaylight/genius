    ...
    output {
        ...
        public Action buildAction(int newActionKey, ActionInfo actionInfo) {
            if (actionInfo instanceof ActionOutput) {
                ...
                long portNum = Long.parseLong(actionValues[0]);
                int maxLength = actionValues.length == 2 ? Integer.parseInt(actionValues[1]) : 0;
                return new ActionOutput(portNum, maxLength).buildAction(newActionKey);
            }
        }
    },
