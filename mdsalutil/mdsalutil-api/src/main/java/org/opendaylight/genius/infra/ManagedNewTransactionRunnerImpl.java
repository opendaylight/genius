...
/**
 * TODO FIX this old and now wrong documentation!
 * ManagedTransactionRunner which offers new single transactions, every time.
 * This implementation is only suitable for dumb clients who are not interested
 * in receiving the Future returned from the {@link WriteTransaction#submit()}.
 * Operations are asynchronous, so upon returning from method here, the
 * requested operation may still be ongoing in the background. Errors from
 * submit are logged.
 */
