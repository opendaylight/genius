...
public class MatchEthernetDestination extends MatchInfoHelper<EthernetMatch, EthernetMatchBuilder> {
    ...
    public MatchEthernetDestination(MacAddress address) {
        super(MatchFieldType.eth_dst, new String[] {address.getValue()});
     ...
     */
    @Deprecated
    public MatchEthernetDestination(MacAddress address, BigInteger[] bigMatchValues, MatchFieldType matchField,
    ...
    protected void populateBuilder(EthernetMatchBuilder builder) {
        builder.setEthernetDestination(new EthernetDestinationBuilder().setAddress(address).build());
    }
