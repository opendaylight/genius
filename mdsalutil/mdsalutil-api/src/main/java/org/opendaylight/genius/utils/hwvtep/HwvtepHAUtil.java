...
import static org.opendaylight.genius.utils.hwvtep.HwvtepSouthboundConstants.TEP_PREFIX;
...
public class HwvtepHAUtil {
    ...
    public static NodeId getNodeId(String haId) {
        String nodeString = HwvtepSouthboundConstants.HWVTEP_URI_PREFIX + "://" +
                HwvtepSouthboundConstants.UUID + "/" + java.util.UUID.nameUUIDFromBytes(haId.getBytes()).toString();
    ...
    public static InstanceIdentifier<Node> getInstanceIdentifier(String haUUidVal) {
        String nodeString = HwvtepSouthboundConstants.HWVTEP_URI_PREFIX + "://" +
                HwvtepSouthboundConstants.UUID + "/" + java.util.UUID.nameUUIDFromBytes(haUUidVal.getBytes()).toString();
        NodeId nodeId = new NodeId(new Uri(nodeString));
    ...
    public static Optional<Node> readOptionalNode(ReadWriteTransaction tx, LogicalDatastoreType storeType, InstanceIdentifier<Node> nodeId)
            throws ReadFailedException, ExecutionException, InterruptedException {
    ...
    public static Node readNode(ReadWriteTransaction tx, LogicalDatastoreType storeType, InstanceIdentifier<Node> nodeId)
            throws ReadFailedException, ExecutionException, InterruptedException {
        Optional<Node> optional = tx.read(storeType, nodeId).get();
    ...
    public static List<String> getNodeIdsForSwitchNode(DataBroker db, Node switchNode) {
        ...
        if (!haPSNodeId.contains(HwvtepSouthboundConstants.PSWITCH_URI_PREFIX)) {
    ...
    static InstanceIdentifier<Node> getGlobalPathFromPSPath(InstanceIdentifier<Node> psNodPath) {
        ...
        InstanceIdentifier<Node> nodePath = HwvtepHAUtil.createInstanceIdentifier(nodePathVal);
        return nodePath;
    }
    ...
    public static InstanceIdentifier<Node> getPSpath(Node psNode, InstanceIdentifier<Node> nodePath) {
        String psNodeId = psNode.getNodeId().getValue();
        String psName = psNodeId.substring(psNodeId.indexOf(HwvtepSouthboundConstants.PHYSICALSWITCH) + HwvtepSouthboundConstants.PHYSICALSWITCH.length());
        String haPsNodeIdVal = nodePath.firstKeyOf(Node.class).getNodeId().getValue() + HwvtepSouthboundConstants.PHYSICALSWITCH + psName;
        InstanceIdentifier<Node> haPsPath = HwvtepHAUtil.createInstanceIdentifier(haPsNodeIdVal);
        return haPsPath;
    }

    public static NodeBuilder getGlobalNodeBuilder(InstanceIdentifier<Node> haPath, HwvtepGlobalAugmentation augmentation) {
        NodeBuilder nodeBuilder = new NodeBuilder();
        nodeBuilder.setNodeId(haPath.firstKeyOf(Node.class).getNodeId());
        nodeBuilder.addAugmentation(HwvtepGlobalAugmentation.class, augmentation);
        return nodeBuilder;
    }

    public static NodeBuilder getNodeBuilder(InstanceIdentifier<Node> haPath) {
        NodeBuilder nodeBuilder = new NodeBuilder();
        nodeBuilder.setNodeId(haPath.firstKeyOf(Node.class).getNodeId());
        return nodeBuilder;
    }

    public static HwvtepGlobalAugmentation getGlobalAugmentation(Node node) {
    ...
    public static PhysicalSwitchAugmentation getPhysicalSwitchAugmentation(Node psNode) {
    ...
    public static String getHAChildren(Node node) {
        ...
        if (globalAugmentation != null) {
            ...
            if (managers != null && managers.size() > 0 && managers.get(0).getManagerOtherConfigs() != null) {
                for (ManagerOtherConfigs configs : managers.get(0).getManagerOtherConfigs()) {
                    if (configs.getOtherConfigKey().equals(HwvtepSouthboundConstants.HA_CHILDREN)) {
    ...
    public static String getHAParentId(Node node) {
        NodeId hwvtepNodeId = null;
        if (node.getAugmentation(HwvtepGlobalAugmentation.class) == null) {
            return null;
        }
        ...
        if (globalAugmentation != null) {
            ...
            if (managers != null && managers.size() > 0 && managers.get(0).getManagerOtherConfigs() != null) {
                for (ManagerOtherConfigs configs : managers.get(0).getManagerOtherConfigs()) {
                    if (configs.getOtherConfigKey().equals(HwvtepSouthboundConstants.HA_ID)) {
                        return configs.getOtherConfigValue();
                    }
                }
            }
    ...
    public static  List<NodeId> getChildNodeIds(Optional<Node> haGlobalConfigNodeOptional) {
        ...
        if (augmentation != null && augmentation.getManagers() != null
            ...
            for (ManagerOtherConfigs otherConfigs : managers.getManagerOtherConfigs()) {
                if (otherConfigs.getOtherConfigKey().equals(HwvtepSouthboundConstants.HA_CHILDREN)) {
