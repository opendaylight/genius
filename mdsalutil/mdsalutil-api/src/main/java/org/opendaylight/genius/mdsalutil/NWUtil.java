...
public final class NWUtil {
    ...
    */
     ...
     */
    public static boolean isIpAddressInRange(IpAddress ipAddress, String cidr) {
        try {
            String ip = String.valueOf(ipAddress.getValue());
            if (ipAddress.getIpv4Address() != null) {
                SubnetUtils subnetUtils = new SubnetUtils(cidr);
                return subnetUtils.getInfo().isInRange(ip);
            } else if (ipAddress.getIpv6Address() != null) {
                return isIpAddressInRange(ipAddress.getIpv6Address(), cidr);
            }
        } catch (IllegalArgumentException e) {
            LOG.error("isIpAddressInRange: ipAddress={}, cidr={}", ipAddress, cidr, e);
        }
        return false;
    }
     ...
     * @param cidr the cidr
     * @return true, if IPv6 address in range
     */
    public static boolean isIpAddressInRange(Ipv6Address ipv6Address, String cidr) {
        String[] arrayCidr = cidr.split("/");
        if (arrayCidr.length < 2) {
            throw new IllegalArgumentException("Invalid CIDR: " + cidr);
        }
        String subnet = arrayCidr[0];
        int bits = Integer.valueOf(arrayCidr[1]);
        try {
            // translate our range to an IP address represented by a byte array
            byte[] ipRangeBytes = InetAddress.getByName(subnet).getAddress();
            // now turn that byte array into an integer
            BigInteger range = new BigInteger(ipRangeBytes);
            ...
            BigInteger ip = new BigInteger(InetAddress.getByName(ipv6Address.getValue()).getAddress());
            if (lowIp.compareTo(ip) <= 0 && highIp.compareTo(ip) >= 0) {
                return true;
            }
        } catch (UnknownHostException e) {
            LOG.error("Unknown host for ipAddress={}, cidr={} ", ipv6Address.getValue(), cidr, e);
        }
        return false;
    }
