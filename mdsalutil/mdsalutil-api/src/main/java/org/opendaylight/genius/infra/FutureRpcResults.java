...
public final class FutureRpcResults {
    ...
    public static final class FutureRpcResultBuilder<I, O> implements Builder<Future<RpcResult<O>>> {
        ...
        private Optional<Function<Throwable, String>> optRpcErrorMessageFunction = Optional.empty();
        private Optional<Consumer<O>> optOnSuccessConsumer = Optional.empty();
        private Optional<Consumer<Throwable>> optOnFailureConsumer = Optional.empty();
            ...
            FutureCallback<O> callback = new FutureCallback<O>() {
                ...
                public void onSuccess(O result) {
                    onSuccessLogLevel.log(logger, "RPC {}() successful; input = {}, output = {}", rpcMethodName,
                            input, result);
                    optOnSuccessConsumer.ifPresent(onSuccessConsumer -> onSuccessConsumer.accept(result));
                ...
                public void onFailure(Throwable cause) {
                    ...
                    optOnFailureConsumer.ifPresent(onFailureConsumer -> onFailureConsumer.accept(cause));
                    RpcResultBuilder<O> rpcResultBuilder = RpcResultBuilder.<O>failed().withError(APPLICATION,
                            optRpcErrorMessageFunction.orElse(Throwable::getMessage).apply(cause), cause);
        ...
        public FutureRpcResultBuilder<I,O> onFailure(Consumer<Throwable> newOnFailureConsumer) {
            this.optOnFailureConsumer.ifPresent(f -> {
                throw new IllegalStateException("OnFailure can only be set once");
            });
            this.optOnFailureConsumer = Optional.of(newOnFailureConsumer);
            return this;
        }
        ...
        public FutureRpcResultBuilder<I,O> withRpcErrorMessage(Function<Throwable, String> newRpcErrorMessageFunction) {
            this.optRpcErrorMessageFunction.ifPresent(f -> {
                throw new IllegalStateException("withRpcErrorMessage can only be set once");
            });
            this.optRpcErrorMessageFunction = Optional.of(newRpcErrorMessageFunction);
            return this;
        }
        ...
        public FutureRpcResultBuilder<I,O> onSuccess(Consumer<O> newOnSuccessFunction) {
            this.optOnSuccessConsumer.ifPresent(f -> {
                throw new IllegalStateException("OnSuccess can only be set once");
            });
            this.optOnSuccessConsumer = Optional.of(newOnSuccessFunction);
            return this;
        }
