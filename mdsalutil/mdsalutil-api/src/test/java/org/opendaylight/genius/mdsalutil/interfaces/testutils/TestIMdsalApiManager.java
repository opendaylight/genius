        ...
        boolean add(E element) {
            for (int i = 0; i < list.size(); i++) {
                ...
                if (equalsStrategy.equals(element, inList)) {
                    String msg;
                    if (element.equals(inList)) {
                        msg = "adding an element that already has an equal entry in list; "
                                + "replacing! ({})";
                    } else {
                        msg = "adding an element that already has a (functionally) equivalent entry in list; "
                                + "replacing! (old: {}, new: {})";
                    }
                    LOG.warn(msg, inList, element);
                    list.set(i, element);
                    return true;
                }
            }
