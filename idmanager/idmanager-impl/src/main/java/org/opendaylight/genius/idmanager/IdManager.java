...
public class IdManager implements IdManagerService, IdManagerMonitor {
    ...
    private long getIdsFromOtherChildPools(ReleasedIdsHolderBuilder releasedIdsBuilderParent, IdPool parentIdPool)
        ...
        List<Map.Entry<ChildPoolsKey, ChildPools>> list = new LinkedList<>(childPoolsList.entrySet());
        list.sort(Comparator.comparing(o -> o.getKey().toString()));
        for (Map.Entry<ChildPoolsKey, ChildPools> aa : list) {
            childPoolsList.put(aa.getKey(), aa.getValue());
        }
        /**
        childPoolsList.entrySet().stream().sorted()
        childPoolsList.entrySet().stream()
