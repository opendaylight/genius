...
public class InterfaceInventoryStateTaskSequencer implements SequencerTask {
    ...
    public InterfaceInventoryStateTaskSequencer(final JobCoordinator coordinator) {
        executorService = Executors.newListeningSingleThreadExecutor(
                "InterfaceInventorySequencer", LOG);
    ...
    private class InterfaceTimerTask extends TimerTask {
        ...
        public void run() {
            ...
            while (!timerCleared) {
                NamedSimpleReentrantLock.AcquireResult lock = interfaceLock.tryAcquire(interfaceName,
                        LOCK_WAIT_TIME_IN_MILLISECS, TimeUnit.MILLISECONDS);
                try {
                    if (!lock.wasAcquired()) {
        ...
        public void run() {
            while (true) {
                for (String infName : jobQueueMap.keySet()) {
                    NamedSimpleReentrantLock.AcquireResult lock = interfaceLock.tryAcquire(infName,
                            LOCK_WAIT_TIME_IN_MILLISECS, TimeUnit.MILLISECONDS);
                    try {
                        if (!lock.wasAcquired()) {
    ...
    public void enqueueJob(String key, Callable<List<ListenableFuture<Void>>> task, int maxRetries) {
        ...
        NamedSimpleReentrantLock.AcquireResult lock = interfaceLock.tryAcquire(key,
                LOCK_WAIT_TIME_IN_MILLISECS, TimeUnit.MILLISECONDS);
