...
public class FlowBasedIngressServicesConfigBindHelper extends AbstractFlowBasedServicesConfigBindHelper {
    ...
    protected List<ListenableFuture<Void>> bindServiceOnTunnel(BoundServices boundServiceNew,
        ...
        if (!isCurrentServiceHighestPriority) {
            FlowBasedServicesUtils.installLPortDispatcherFlow(dpId, boundServiceNew, ifState.getName(), transaction,
                    ifState.getIfIndex(), boundServiceNew.getServicePriority(),
    ...
    @Override
    protected List<ListenableFuture<Void>> bindServiceOnInterfaceType(BoundServices boundServiceNew, List<BoundServices> allServices, DataBroker dataBroker) {
        return null;
    }

    protected List<ListenableFuture<Void>> bindServiceOnVlan(BoundServices boundServiceNew,
            List<BoundServices> allServices,
            org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.interfaces.rev140508.interfaces.state
            .Interface ifState, DataBroker dataBroker) {
        List<ListenableFuture<Void>> futures = new ArrayList<>();
