...
public final class FlowBasedServicesUtils {
    ...
    public static void installInterfaceIngressFlow(Uint64 dpId, Interface iface, BoundServices boundServiceNew,
        ...
        if (vlanId != 0) {
            ...
            instructionSet.put(MDSALUtil.buildAndGetPopVlanActionInstruction(actionKey, ++serviceInstructionsSize).key(),
                    MDSALUtil.buildAndGetPopVlanActionInstruction(actionKey, ++serviceInstructionsSize));
        }
        ...
        if (lportTag != 0L) {
            ...
            Uint64 metadataMask = MetaDataUtil.getMetaDataMaskForLPortDispatcher(
                    ...
                    metadataValues[1]);
            instructionSet.put(
                    MDSALUtil.buildAndGetWriteMetadaInstruction(metadata, metadataMask, ++serviceInstructionsSize).key(),
    ...
    private static void installEgressDispatcherFlow(Uint64 dpId, BoundServices boundService, String interfaceName,
        ...
        if (boundService.getServicePriority().toJava() != ServiceIndex.getIndex(NwConstants.DEFAULT_EGRESS_SERVICE_NAME,
            ...
            instructions.put(MDSALUtil.buildAndGetWriteMetadaInstruction(metadataValues[0], metadataMask,
                    instructions.size()).key(),MDSALUtil.buildAndGetWriteMetadaInstruction(metadataValues[0], metadataMask,
                    instructions.size()));
    ...
    public static void installLportIngressFlow(Uint64 dpId, long portNo, Interface iface,
        ...
        instructions.put(MDSALUtil.buildAndGetGotoTableInstruction(NwConstants.LPORT_DISPATCHER_TABLE, instructionKey++).key(),
