...
public final class FlowBasedServicesUtils {
    ...
    public static void installInterfaceIngressFlow(Uint64 dpId, Interface iface, BoundServices boundServiceNew,
        ...
        if (vlanId != 0) {
            ...
            instructionSet.values().add(MDSALUtil.buildAndGetPopVlanActionInstruction(actionKey, ++serviceInstructionsSize));
        }
        ...
        if (instructions != null && !instructions.isEmpty()) {
            for (Instruction info : instructions.values()) {
                ...
                instructionSet.values().add(info);
            }
        }
    ...
    public static void installLPortDispatcherFlow(Uint64 dpId, BoundServices boundService, String interfaceName,
        ...
        instructions.values().add(MDSALUtil.buildAndGetWriteMetadaInstruction(metadata, metadataMask, ++instructionSize));
    ...
    private static void installEgressDispatcherFlow(Uint64 dpId, BoundServices boundService, String interfaceName,
        ...
        if (boundService.getServicePriority().toJava() != ServiceIndex.getIndex(NwConstants.DEFAULT_EGRESS_SERVICE_NAME,
            ...
            instructions.values().add(MDSALUtil.buildAndGetWriteMetadaInstruction(metadataValues[0], metadataMask,
                    instructions.size()));
        ...
        for (Instruction info : serviceInstructions.values()) {
            if (info.getInstruction() instanceof WriteActionsCase) {
                List<Action> writeActions = ActionConverterUtil.convertServiceActionToFlowAction(
                        ((WriteActionsCase) info.getInstruction()).getWriteActions().getAction());
                instructions.values().add(MDSALUtil.buildWriteActionsInstruction(writeActions, instructions.size()));
            } else if (info.getInstruction() instanceof ApplyActionsCase) {
                ...
                // Skip meta data write as that is handled already
                instructions.values().add(MDSALUtil.buildInstruction(info, instructions.size()));
            }
        }
        if (!finalApplyActions.isEmpty()) {
            instructions.values().add(MDSALUtil.buildApplyActionsInstruction(finalApplyActions, instructions.size()));
        }
    ...
    public static void installLportIngressFlow(Uint64 dpId, long portNo, Interface iface,
        ...
        if (!actions.isEmpty()) {
            instructions.values().add(MDSALUtil.buildApplyActionsInstruction(actions, instructionKey++));
        }
        ...
        instructions.values().add(MDSALUtil.buildAndGetWriteMetadaInstruction(metadata, metadataMask, instructionKey++));
        instructions.values()
