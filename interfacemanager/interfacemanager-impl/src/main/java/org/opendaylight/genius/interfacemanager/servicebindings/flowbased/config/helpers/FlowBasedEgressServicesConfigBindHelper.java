...
import org.opendaylight.yang.gen.v1.urn.opendaylight.genius.interfacemanager.servicebinding.rev160406.service
    .bindings.services.info.BoundServices;
...
public class FlowBasedEgressServicesConfigBindHelper extends AbstractFlowBasedServicesConfigBindHelper {
    ...
    protected List<ListenableFuture<Void>> bindServiceOnTunnel(BoundServices boundServiceNew, List<BoundServices>
        allServices, Interface ifState, DataBroker dataBroker) {
        LOG.info("bind service on egress tunnel interface - WIP");
        return null;
    }

    protected List<ListenableFuture<Void>> bindServiceOnVlan(BoundServices boundServiceNew, List<BoundServices>
        allServices, Interface ifState, DataBroker dataBroker) {
        ...
        if (allServices.size() == 1) {
            ...
            FlowBasedServicesUtils.installEgressDispatcherFlows(dpId, boundServiceNew, ifState.getName(),
                    transaction, ifState.getIfIndex(), NwConstants.DEFAULT_SERVICE_INDEX, (short) (boundServiceNew
                    .getServicePriority() + 1), iface);
            if (transaction != null) {
        ...
        short nextServiceIndex = ServiceIndex.getIndex(NwConstants.DEFAULT_EGRESS_SERVICE_NAME, NwConstants
            .DEFAULT_EGRESS_SERVICE_INDEX); // dummy service index
        if (low != null) {
            ...
            if (low.equals(highest)) {
                ...
                short lowerServiceIndex = (short) ((lower != null) ? lower.getServicePriority() : low
                    .getServicePriority() + 1);
