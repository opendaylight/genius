...
public class FlowBasedEgressServicesConfigBindHelper implements FlowBasedServicesConfigAddable {
    ...
    private static List<ListenableFuture<Void>> bindServiceOnVlan(BoundServices boundServiceNew, List<BoundServices> allServices,
        ...
        short nextServiceIndex = IfmConstants.LEAST_PRIORITY_SERVICE_INDEX; // dummy service index
        if (low != null) {
        ...
        if (high != null) {
            ...
            if (high.equals(highest)) {
                FlowBasedServicesUtils.installEgressDispatcherFlow(dpId, high, ifState.getName(), transaction, ifState.getIfIndex(), IfmConstants.DEFAULT_SERVICE_INDEX, currentServiceIndex);
            } else {
