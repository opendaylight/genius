...
public class FlowBasedIngressServicesConfigUnbindHelper extends AbstractFlowBasedServicesConfigUnbindHelper {
    ...
    protected List<ListenableFuture<Void>> unbindServiceOnVlan(BoundServices boundServiceOld,
        ...
        if (high == null) {
            LOG.trace("Deleting table entry for service {}, match service index {}", boundServiceOld,
                    NwConstants.DEFAULT_SERVICE_INDEX);
            FlowBasedServicesUtils.removeLPortDispatcherFlow(dpId, ifaceState.getName(), boundServiceOld, tx,
                    NwConstants.DEFAULT_SERVICE_INDEX);
            if (low != null) {
                //delete the lower services flow entry.
                LOG.trace("Deleting table entry for lower service {}, match service index {}", low,
                ...
                LOG.trace("Installing new entry for lower service {}, match service index {}, update service index {}",
    ...
    protected List<ListenableFuture<Void>> unbindServiceOnTunnel(BoundServices boundServiceOld,
        ...
        FlowBasedServicesUtils.installInterfaceIngressFlow(dpId, iface, toBeMoved, tx,
    ...
    @Override
    protected List<ListenableFuture<Void>> unbindServiceOnInterfaceType(BoundServices boundServiceNew, List<BoundServices> allServices, DataBroker dataBroker) {
        //TODO
        return null;
    }
