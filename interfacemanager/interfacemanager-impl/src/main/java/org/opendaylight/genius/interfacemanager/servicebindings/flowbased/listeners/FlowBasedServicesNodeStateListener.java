...
public class FlowBasedServicesNodeStateListener
    ...
    private class RendererStateInterfaceBindWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        public List<ListenableFuture<Void>> call() {
            ...
            if (servicesInfo == null) {
                LOG.trace("service info is null for interface type {}", iface);
                return null;
            }

            List<BoundServices> allServices = servicesInfo.getBoundServices();
            if (allServices == null || allServices.isEmpty()) {
                LOG.trace("bound services is empty for interface type {}", iface);
                return null;
            }
            return flowBasedServicesStateAddable.bindServicesOnInterfaceType(dpnId, iface, allServices, broker);
        }
    }
    ...
    private class RendererStateInterfaceUnbindWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        public List<ListenableFuture<Void>> call() {
            ...
            return flowBasedServicesStateRemovable.unbindServicesOnInterfaceType(dpnId, iface, allServices, broker);
        }
    }
}
