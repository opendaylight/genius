...
public class FlowBasedServicesConfigListener implements ClusteredDataTreeChangeListener<ServicesInfo> {
    ...
    public void onDataTreeChanged(@Nonnull Collection<DataTreeModification<ServicesInfo>> collection) {
        for (DataTreeModification<ServicesInfo> dataTreeModification : collection) {
            InstanceIdentifier<ServicesInfo> rootIdentifier = dataTreeModification.getRootPath().getRootIdentifier();
            DataObjectModification<ServicesInfo> rootNode = dataTreeModification.getRootNode();
