    ...
    public static List<ListenableFuture<Void>> removeConfiguration(ManagedNewTransactionRunner txRunner,
        ...
        if (globalNodeId != null) {
            ...
            futures.add(txRunner.callWithNewWriteOnlyTransactionAndSubmit(
                configTx -> removeTerminationEndPoint(configTx, ifTunnel, globalNodeId)));
            // Default operational
            futures.add(txRunner.callWithNewWriteOnlyTransactionAndSubmit(operTx -> {
                InterfaceManagerCommonUtils.deleteStateEntry(interfaceOld.getName(), operTx);
                InterfaceMetaUtils.removeTunnelToInterfaceMap(physicalSwitchNodeId, operTx, ifTunnel);
            }));
        }
        return futures;
    }
