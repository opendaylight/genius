...
public abstract class AbstractFlowBasedServicesConfigBindHelper implements FlowBasedServicesConfigAddable {
     ...
     */
    public AbstractFlowBasedServicesConfigBindHelper(DataBroker dataBroker, InterfacemgrProvider interfacemgrProvider) {
        this.dataBroker = dataBroker;
        this.interfacemgrProvider = interfacemgrProvider;
    }

    @Override
    public void bindService(List<ListenableFuture<Void>> futures, String interfaceName, BoundServices boundServiceNew,
                            List<BoundServices> allServices, BoundServicesState interfaceBoundServicesState) {
        ...
        if (FlowBasedServicesUtils.isInterfaceTypeBasedServiceBinding(interfaceName)) {
            bindServiceOnInterfaceType(futures,boundServiceNew, allServices, dataBroker);
        } else {
            if (L2vlan.class.equals(interfaceBoundServicesState.getInterfaceType())
                ...
                bindServiceOnInterface(futures, boundServiceNew, allServices, interfaceBoundServicesState, dataBroker);
            }
        }
    }
