...
import static org.opendaylight.genius.alivenessmonitor.internal.AlivenessMonitorUtil.*;
...
//                    }
        ...
        try {
            ...
            if(optKey.isPresent()) {
                ...
                if (ethType == EtherTypes.Bfd) {
                    ...
                    monitoringState = new MonitoringStateBuilder().setMonitorKey(monitoringKey).setMonitorId(monitorId)
                                                      .setState(LivenessState.Unknown).setStatus(MonitorStatus.Started).setRequestCount(INITIAL_COUNT)
                        ...
                        public void onSuccess(Optional<MonitorProfile> optProfile) {
                            ...
                            if(optProfile.isPresent()) {
                                ...
                                if (protocolType == EtherTypes.Bfd) {
                                    ...
                                    ((HwVtepTunnelsStateHandler) ethTypeToProtocolHandler.get(protocolType)).resetMonitoringTask(info, true);
                                } else {
    ...
    private boolean stopMonitoringTask(Long monitorId, boolean interruptTask) {
        ...
        Optional<MonitorProfile> optProfile = read(LogicalDatastoreType.OPERATIONAL, getMonitorProfileId(monitoringInfo.getProfileId()));
        EtherTypes protocolType = optProfile.get().getProtocolType();
        if (protocolType == EtherTypes.Bfd) {
            LOG.debug("disabling bfd for hwvtep tunnel montior id {}", monitorId);
            ((HwVtepTunnelsStateHandler) ethTypeToProtocolHandler.get(protocolType)).resetMonitoringTask(monitoringInfo, false);
            return true;
        }
