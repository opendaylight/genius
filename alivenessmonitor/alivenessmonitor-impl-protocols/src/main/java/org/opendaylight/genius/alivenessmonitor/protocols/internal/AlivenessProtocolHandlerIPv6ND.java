...
public class AlivenessProtocolHandlerIPv6ND extends AbstractAlivenessProtocolHandler<Packet> {
    ...
    public String handlePacketIn(Packet packet, PacketReceived packetReceived) {
        ...
        if (!StringUtils.isNotBlank(interfaceName)) {
            String sourceIp = naPacket.getSourceIpv6().getValue();
            String targetIp = naPacket.getTargetAddress().getValue();
    ...
    public void startMonitoringTask(MonitoringInfo monitorInfo) {
        ...
        final PhysAddress srcMacAddress = Preconditions.checkNotNull(getMacAddress(source),
        ...
        Futures.addCallback(JdkFutureAdapters.listenInPoolThread(future),
                new FutureCallback<RpcResult<SendNeighborSolicitationOutput>>() {
                    @Override
    ...
    private String getIpAddress(EndpointType source) {
        ...
        if (source instanceof IpAddress) {
            ipAddress = String.valueOf(((IpAddress) source).getIpAddress().getValue());
        } else if (source instanceof Interface) {
            ipAddress = String.valueOf(((Interface) source).getInterfaceIp().getValue());
        }
        return ipAddress;
    }
